...
describe('U23 F20 import, export, and portable backup wireframe', () => {
  ...
  it('builds safe versioned CSV and JSON export previews', () => {
    ...
    expect(JSON.stringify(fixture.exportPreview({ format: 'json', includeAttachments: true }))).not.toMatch(
      /token|oauth|service.?key|security.?log|deleted.?other/i,
    );
  });
  ...
  ] as [ImportExportScenario, string, string][])('inspects %s file profile safely', (scenario, encoding, delimiter) => {
    expect(createImportExportFixture(scenario).inspectFile()).toMatchObject({ encoding, delimiter });
  });

  it('parses locale amounts exactly and escapes formula cells as text', () => {
    expect(parseImportAmount('1.250.000', 'id-ID')).toEqual({ minor: '125000000', currency: 'IDR' });
    ...
    const fixture = createImportExportFixture('ready');
    expect(fixture.formulaCell('=HYPERLINK("x")')).toMatchObject({ escaped: true, treatedAsText: true });
  });

  it('maps columns and previews valid/error rows with safe reasons', () => {
    const fixture = createImportExportFixture('ready');
    expect(fixture.mapColumns({ date: 'Tanggal', amount: 'Jumlah', currency: 'Mata Uang' })).toMatchObject({
      kind: 'mapped',
      requiredComplete: true,
    });
    expect(fixture.previewRows()).toMatchObject({ validCount: 2, errorCount: 3, reasonsVisible: true });
    expect(JSON.stringify(fixture.previewRows())).not.toMatch(/merchant|note|account.?id|transaction.?id/i);
  });
  ...
  it('requires review for duplicates, invalid transfers, and missing required fields', () => {
    ...
    expect(fixture.duplicatePolicy('review')).toMatchObject({ requiresReview: true, autoDelete: false });
  ...
  it('supports bounded idempotent import progress, resume, rollback, and reconciliation', () => {
    ...
    expect(fixture.progress()).toMatchObject({ committedCount: 4, rejectedCount: 2, resumable: true });
  ...
  it('keeps cancel/expiry/offline/kill-switch and privacy deletion handoff safe', () => {
    expect(createImportExportFixture('ready').cancel()).toMatchObject({ localPurged: true, serverResult: null });
    expect(createImportExportFixture('offline').offlineState()).toMatchObject({ localPreview: true, commitBlocked: true });
    expect(createImportExportFixture('expired').expiry()).toMatchObject({ cleaned: true, shareAvailable: false });
    expect(createImportExportFixture('kill_switch').killSwitch()).toMatchObject({ importDisabled: true, coreExportAvailable: true });
    expect(createImportExportFixture('ready').privacyDeletionHandoff()).toMatchObject({ route: '/profile', safe: true });
  });
  ...
  it('does not use network, persistence, logging, or sensitive navigation data', () => {
    ...
    expect(fixture.diagnosticMetadata()).toMatchObject({ payloadIncluded: false, networkCalled: false });
    expect(JSON.stringify(fixture.safeRoute('preview'))).not.toMatch(/amount|filename|content|job.?id|token/i);
