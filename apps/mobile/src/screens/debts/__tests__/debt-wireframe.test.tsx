...
describe('U17 F14 debt and loans wireframe', () => {
  ...
  it('validates debt type, currency, non-negative opening balance, and dates', () => {
    ...
    expect(validateDebtDraft({ ...validDraft, currency: 'USD' })).toContain('Currency fixture tidak didukung.');
    expect(validateDebtDraft({ ...validDraft, name: '' })).toContain('Nama debt harus 1–80 karakter.');
    expect(validateDebtDraft({ ...validDraft, openingAsOf: '2026-02-31' })).toContain('Tanggal fixture tidak valid.');
    expect(validateDebtDraft({ ...validDraft, dueDate: '2025-12-01' })).toContain('Due date tidak boleh sebelum as-of.');
    expect(validateDebtDraft({ ...validDraft, kind: 'credit_card', creditLimitMinor: '0' })).toContain('Limit harus positif bila diisi.');
  });

  it('separates actual, pending, forecast, statement mode, partial FX, and credit balance', () => {
    expect(calculateOutstanding('10000000', '800000', '300000', 'posted')).toEqual({ actual: '9200000', pending: '0', projected: '9200000' });
    expect(calculateOutstanding('10000000', '800000', '300000', 'pending')).toEqual({ actual: '10000000', pending: '800000', projected: '9200000' });
    expect(calculateOutstanding('100000', '150000', '0', 'posted')).toMatchObject({ actual: '-50000', creditBalance: '50000' });
    expect(createDebtFixture('partial_fx').summary()).toMatchObject({ nativeVisible: true, baseAggregate: 'partial', missingFxCount: 1 });
    expect(createDebtFixture('statement_assisted').summary()).toMatchObject({ trackingMode: 'statement_assisted', reconciliation: 'unreconciled' });
  });

  it('reconciles payment components atomically and excludes principal from expense', () => {
    expect(reconcilePayment({ total: '1000000', principal: '800000', interest: '150000', fee: '50000', adjustment: '0' })).toMatchObject({ valid: true, principalIsExpense: false, expenseMinor: '200000' });
    expect(reconcilePayment({ total: '1000000', principal: '800000', interest: '100000', fee: '50000', adjustment: '0' })).toMatchObject({ valid: false, reason: 'total_mismatch' });
    expect(createDebtFixture('pending').paymentResult()).toMatchObject({ actualUnchanged: true, status: 'pending' });
    expect(createDebtFixture('offline').paymentResult()).toMatchObject({ status: 'queued', idempotent: true });
    expect(createDebtFixture('conflict').paymentResult()).toMatchObject({ status: 'review_required' });
    expect(createDebtFixture().paymentResult()).toMatchObject({ principalTransfer: true, interestExpense: true, feeExpense: true, atomic: true });
  });

  it('supports statement difference and explicit adjustment confirmation', () => {
    expect(statementReconciliation('9500000', '9400000')).toEqual({ difference: '100000', status: 'needs_confirmation', applied: false });
    expect(createDebtFixture().reconcileStatement(false)).toMatchObject({ applied: false, openingChanged: false });
    expect(createDebtFixture().reconcileStatement(true)).toMatchObject({ applied: true, openingChanged: false, adjustment: '100000' });
    expect(createDebtFixture('statement_mismatch').reconcileStatement(false)).toMatchObject({ status: 'needs_confirmation' });
  });

  it('generates deterministic fixed-rate schedules including zero APR, rounding, and negative amortization', () => {
    const zero = amortizationSchedule({ principal: '1200000', aprBps: 0, periods: 12, paymentsPerYear: 12, scheduledPayment: null, periodicFee: '0' });
    expect(zero[0]).toMatchObject({ principal: '100000', interest: '0' });
    expect(zero.at(-1)?.closingPrincipal).toBe('0');
    expect(amortizationSchedule({ principal: '1000000', aprBps: 1200, periods: 2, paymentsPerYear: 12, scheduledPayment: '1000', periodicFee: '0' }).negativeAmortization).toBe(true);
    expect(amortizationSchedule({ principal: '1000001', aprBps: 0, periods: 3, paymentsPerYear: 12, scheduledPayment: null, periodicFee: '0' }).at(-1)?.closingPrincipal).toBe('0');
    expect(createDebtFixture('amortization_kill_switch').schedule()).toMatchObject({ visible: false, kind: 'maintenance' });
    expect(createDebtFixture('too_large_schedule').schedule()).toMatchObject({ visible: false, kind: 'too_large' });
  });
  ...
  it('keeps archive/history, reminders, scenarios, and deterministic recovery states', () => {
    ...
    expect(createDebtFixture('reminder_kill_switch').reminder()).toMatchObject({ kind: 'in_app_only' });
    expect(fixture.extraPrincipal('200000')).toMatchObject({ kind: 'scenario', actualChanged: false });
    expect(fixture.retry()).toMatchObject({ kind: 'refreshed' });
    expect(createDebtFixture('materialization_failure').retry()).toMatchObject({ kind: 'incremental_retry' });
  });

  it.each([
    ['loading', /Memuat debt/], ['empty', /Belum ada debt/], ['offline', /Offline/], ['stale', /forecast stale/],
    ['partial_fx', /FX belum tersedia/], ['statement_mismatch', /Selisih statement/], ['permission_revoked', /read-only/],
    ['archived_closed', /arsip/], ['negative_amortization', /negative amortization/], ['amortization_kill_switch', /maintenance/],
    ['reminder_kill_switch', /in-app/], ['too_large_schedule', /terlalu besar/], ['conflict', /konflik/], ['invalid', /tidak valid/],
