...
const eligibleEntries: ReconciliationEntry[] = [
  { lifecycle: 'posted', clearing: 'cleared', occurredAt: '2026-08-20T10:00:00.000Z', signedMinor: '-200000' },
  { lifecycle: 'posted', clearing: 'reconciled', occurredAt: '2026-08-22T10:00:00.000Z', signedMinor: '500000' },
  { lifecycle: 'draft', clearing: 'cleared', occurredAt: '2026-08-23T10:00:00.000Z', signedMinor: '900000' },
  { lifecycle: 'posted', clearing: 'pending', occurredAt: '2026-08-24T10:00:00.000Z', signedMinor: '700000' },
];
...
describe('U19 F16 search, review, and reconciliation wireframe', () => {
  ...
  it('keeps saved searches user-only and handles source, race, offline, and unauthorized states', () => {
    const fixture = createSearchReviewFixture();
    expect(fixture.saveSearch({ name: 'Belanja rutin', query: 'kedai', filters: {} })).toMatchObject({ kind: 'saved_fixture' });
    expect(fixture.savedSearches()).toHaveLength(1);
    expect(createSearchReviewFixture('source_changed').resolveReview('review-ocr', 'confirm')).toMatchObject({ kind: 'needs_refresh' });
    expect(createSearchReviewFixture('pending_race').bulkResolvePreview(['review-ocr'])).toMatchObject({ kind: 'blocked_race' });
    expect(createSearchReviewFixture('offline').resolveReview('review-ocr', 'dismiss')).toMatchObject({ kind: 'queued' });
    expect(createSearchReviewFixture('unauthorized').resolveReview('review-ocr', 'confirm')).toMatchObject({ kind: 'unauthorized' });
  });
  ...
  it('keeps review reasons explicit and never auto-merges possible duplicates', () => {
    ...
    expect(items.map((item) => item.reason)).toEqual(expect.arrayContaining([
      'low_confidence_ocr', 'possible_duplicate', 'missing_category', 'missing_fx',
      'stale_pending', 'unmatched_recurring', 'sync_conflict', 'reconciliation_stale',
    ]));
    expect(fixture.resolveReview('review-duplicate', 'merge')).toMatchObject({ kind: 'review_required' });
    expect(fixture.bulkResolvePreview(['review-ocr', 'review-category'])).toMatchObject({ kind: 'preview', allOrNothing: true });
  ...
  it('calculates reconciliation from eligible signed account lines and explicit adjustment', () => {
    expect(calculateReconciliation({ openingMinor: '1000000', statementClosingMinor: '1300000', cutoffAt: '2026-08-25T00:00:00.000Z', entries: eligibleEntries })).toMatchObject({
      calculatedClosingMinor: '1300000', differenceMinor: '0', eligibleCount: 2, status: 'balanced',
    });
    expect(calculateReconciliation({ openingMinor: '1000000', statementClosingMinor: '1310000', cutoffAt: '2026-08-25T00:00:00.000Z', entries: eligibleEntries })).toMatchObject({
      differenceMinor: '10000', status: 'in_progress',
    });
    const fixture = createSearchReviewFixture();
    expect(fixture.reconciliation({ openingMinor: '1000000', statementClosingMinor: '1300000', entries: eligibleEntries })).toMatchObject({ kind: 'balanced', excludedPending: 1, excludedDraft: 1 });
    expect(fixture.adjustmentPreview('10000')).toMatchObject({ entryType: 'balance_adjustment', cashflowExcluded: true, signedMinor: '10000' });
  });

  it('keeps reconciliation state safe for stale, offline, and corrupt cursor recovery', () => {
    expect(createSearchReviewFixture('reconciliation_stale').reconciliation({ openingMinor: '0', statementClosingMinor: '0', entries: [] })).toMatchObject({ kind: 'stale' });
    expect(createSearchReviewFixture('offline').finalizeReconciliation()).toMatchObject({ kind: 'offline_disabled' });
    expect(createSearchReviewFixture('corrupt_cursor').retry()).toMatchObject({ kind: 'restarted' });
  });
  ...
  it('supports search, review, reconciliation, bulk preview, back, and safe drill-down actions', () => {
    ...
    for (const label of ['Tanggal', 'Nominal', 'Jenis', 'Status', 'Akun', 'Kategori', 'Tag', 'Mata uang', 'Sumber', 'Struk', 'Catatan']) {
