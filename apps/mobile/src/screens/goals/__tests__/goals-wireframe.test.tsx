...
describe('U15 F12 goals and sinking funds wireframe', () => {
  ...
  it('lists active goals by default and supports lifecycle actions', () => {
    ...
    expect(fixture.list().items).toEqual(expect.arrayContaining([expect.objectContaining({ status: 'active' })]));
    expect(fixture.list().items).not.toEqual(expect.arrayContaining([expect.objectContaining({ status: 'archived' })]));
    expect(fixture.filterStatus('archived').items).toEqual(expect.arrayContaining([expect.objectContaining({ status: 'archived' })]));
  ...
  it('validates draft fields and previews mathematical periodic need', () => {
    ...
    expect(validateGoalDraft({ ...validDraft, name: ' ', targetMinor: '0', currency: 'idr' })).toEqual(
      expect.arrayContaining([expect.stringContaining('Nama'), expect.stringContaining('target'), expect.stringContaining('currency')]),
    );
  ...
  it('computes contribution, withdrawal, pending, remaining, overfunding and milestones with BigInt', () => {
    ...
    expect(calculateGoalProgress({ targetMinor: '1000000', contributionMinor: '100000', withdrawalMinor: '150000', pendingMinor: '0', today: '2026-08-01', deadline: null, cadence: 'monthly' }).visualProgressPercent).toBe(0);
    expect(calculateGoalProgress({ targetMinor: '1000000', contributionMinor: '200000', withdrawalMinor: '0', pendingMinor: '0', today: '2026-08-31', deadline: '2026-08-31', cadence: 'monthly' }).requiredMinor).toBeNull();
    expect(calculateGoalProgress({ targetMinor: '1000000', contributionMinor: '200000', withdrawalMinor: '0', pendingMinor: '0', today: '2026-09-01', deadline: '2026-08-31', cadence: 'weekly' }).status).toBe('past_due_active');
    expect(requiredPeriodicMinor('600000', '2026-08-01', '2026-08-31', 'daily')).toBe('19355');
    expect(requiredPeriodicMinor('600000', '2026-08-01', '2026-08-31', 'weekly')).toBe('150000');
  });

  it('keeps allocation explicit, capped, and separate from income or withdrawal', () => {
    const fixture = createGoalsFixture();
    expect(fixture.businessRules()).toMatchObject({ transferIncome: false, transferCashflow: false, draftCounted: false, voidCounted: false, missingFxCount: 1 });
    expect(fixture.candidateTransfers()).toEqual(expect.arrayContaining([expect.objectContaining({ selectable: true })]));
    expect(fixture.allocate('transfer-candidate', '200000', '500000', true)).toMatchObject({ kind: 'applied', contribution: true, noAutoCount: true });
    expect(fixture.allocate('transfer-candidate', '600000', '500000', true)).toMatchObject({ kind: 'rejected_mutation', reason: expect.stringContaining('melebihi') });
    expect(fixture.allocate('transfer-candidate', '200000', '500000', false)).toMatchObject({ kind: 'review_required' });
  ...
  it('exposes detail/history, offline conflict, and recovery states safely', () => {
    const fixture = createGoalsFixture();
    expect(fixture.detail('goal-travel')).toMatchObject({ formula: expect.stringContaining('remaining'), transactionRoute: '/transactions', milestones: expect.arrayContaining([25, 50, 75, 100]) });
    expect(fixture.targetHistory('goal-travel')).toEqual(expect.any(Array));
    expect(createGoalsFixture('offline').save(validDraft)).toMatchObject({ kind: 'queued', syncStatus: 'pending-sync' });
    expect(createGoalsFixture('conflict').save(validDraft)).toMatchObject({ kind: 'conflict_copy', name: expect.stringContaining('(salinan)') });
    expect(createGoalsFixture('permission_revoked').readOnly()).toMatchObject({ kind: 'read_only' });
    ...
    expect(createGoalsFixture('reminder_kill_switch').reminder()).toMatchObject({ kind: 'manual_only' });
  });

  it('renders every deterministic state, privacy masking, wizard, and live actions', () => {
    const cases: [GoalScenario, RegExp][] = [
      ['loading', /Memuat goal/], ['empty', /Belum ada goal/], ['offline', /Offline/], ['stale', /stale/],
      ['partial', /Bagian ini perlu dicoba lagi/], ['missing_fx', /belum memiliki kurs/], ['invalid_legacy', /tidak dapat digunakan/],
      ['permission_revoked', /read-only/], ['archived_paused', /arsip/], ['overfunded', /overfunded/], ['past_due_active', /past_due_active/],
      ['reminder_kill_switch', /reminder/], ['too_many_lines', /terlalu banyak/],
    ];
